...
function key(...parts){ return parts.map(x => String(x??'').trim().toLowerCase()).join('|'); }

export function readStationsMeta(filePath){
  const wb = XLSX.readFile(filePath);
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['stations_meta']||{}, {defval:null});
  return rows.map(r => ({
...
export function readCurrentPrecip(filePath){
  const wb = XLSX.readFile(filePath);
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['precip']||{}, {defval:null});
  return rows.map(r => ({
...
export function readCurrentWater(filePath){
  const wb = XLSX.readFile(filePath);
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['water_levels']||{}, {defval:null});
  return rows map(r => ({
    date: r.date ? new Date(r.date).toISOString().slice(0,10) : null,
    hour_utc: parseHour(r.hour_utc),
    station_code: String(r.station_code||'').trim(),
...
export function readCurrentAir(filePath){
  const wb = XLSX.readFile(filePath);
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['air_temp']||{}, {defval:null});
  return rows.map(r => ({
    date: r.date ? new Date(r.date).toISOString().slice(0,10) : null,
    hour_utc: parseHour(r.hour_utc),
    station_code: String(r.station_code||'').trim(),
...
export function readHistoricalFile(filePath){
  const wb = XLSX.readFile(filePath);
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['historical']||{}, {defval:null});
  return rows.map(r => ({
    datetime_utc: r.datetime_utc ? new Date(r.datetime_utc).toISOString().slice(0,16)+":00Z" : null,
    station_code: String(r.station_code||'').trim(),
    water_level_cm: num(r.water_level_cm),
    precipitation_mm: num(r.precipitation_mm),
    air_temp_c: num(r.air_temp_c),
...
export function assembleForForecast(meta, water, precip, air){
  ...
  for (const p of precip){
    ...
    if (p.station_code){
      ...
      if (!prev || prev.stamp < stamp) precipByStation.set(p.station_code, { ...p, stamp });
    } else if (p.basin_name){
      const prev = precipByBasin.get(p.basin_name);
      if (!prev || prev.stamp < stamp) precipByBasin.set(p.basin_name, { ...p, stamp });
    }
  }
  ...
  for (const a of air){
    ...
    if (a.station_code){
      ...
      if (!prev || prev.stamp < stamp) airByStation.set(a.station_code, { ...a, stamp });
    }
  }

  const currentInputs = [];
  for (const [k, w] of latestWater.entries()){
    const m = (w.station_code && metaByCode.get(w.station_code)) || metaByName.get(key(w.river_name, w.station_name)) || {};
    const basin = m?.basin_name || '';
    const p = (w.station_code && precipByStation.get(w.station_code)) || (basin && precipByBasin.get(basin)) || null;
  ...
  const settings = meta.map(m => ({
    ...
    x: m.x, y: m.y,
    datum_offset_cm: (m.datum_offset_cm != null ? m.datum_offset_cm : (m.station_level_cm != null ? m.station_level_cm : null)),
